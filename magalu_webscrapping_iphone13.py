...
while True:
    ...
    div_main = driver.find_element(By.XPATH, 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxx') # tem outra opcao
    html_content = div_main.get_attribute('outerHTML')
    soup = BeautifulSoup(html_content, 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
    ...
    try:
        next_button = driver.find_element(By.XPATH, 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxx') 
...
for produto in lista_produtos_total:
    titulo_element = produto.find('h2', class_='xxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
    preco_element = produto.find('p', class_='xxxxxxxxxxxxxxxxxxxxxxxxxxxxx')
